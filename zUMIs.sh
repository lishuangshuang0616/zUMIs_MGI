...
chemistry=$(grep 'chemistry:' ${yaml} | awk '{print $2}')
...
if [[ ${conda} = true ]] ; then
  ...
  zumisenv=${zumisdir}/env/zUMIs-env
  miniconda=${zumisdir}/env/zUMIs-miniconda.tar.bz2
  #check if zUMIs environment has been unpacked from tar
  if [[ ! -d ${zumisenv} ]] || [[ ${zumisdir}/env/zUMIs-miniconda.partaa -nt ${zumisenv} ]] ; then
    [ -d ${zumisenv} ] || mkdir -p ${zumisenv}
    cat ${zumisdir}/env/zUMIs-miniconda.parta* > ${miniconda}
...
${Rexc} ${zumisdir}/src/checkyaml.R ${yaml} > ${project}.zUMIs_YAMLerror.log
...
[ -d ${outdir}/zUMIs_output/expression/downsampling ] || mkdir -p ${outdir}/zUMIs_output/expression/downsampling
...
if [[ "${whichStage}" == "Filtering" ]] ; then
  ...
  if [[ ${f} =~ \.gz$ ]] ; then
      ...
      for i in ${fqfiles} ; do bash ${zumisdir}/src/splitfq.sh ${i} ${pigzexc} ${num_threads} ${tmpMerge} splitfqgz ${project} ${nreads} & done
      wait
      pref=$(basename ${f} .gz)
      ...
      for i in ${fqfiles} ; do bash ${zumisdir}/src/splitfq.sh ${i} ${pigzexc} ${num_threads} ${tmpMerge} splitfq ${project} ${nreads} & done
      wait
      pref=$(basename ${f})
      l=$(ls ${tmpMerge}${pref}* | sed "s|${tmpMerge}${pref}||")
  fi

  for x in ${l} ; do perl ${zumisdir}/src/fqfilter_v2.pl ${yaml} ${samtoolsexc} ${Rexc} ${pigzexc} ${zumisdir} ${x} & done
  wait
  bash ${zumisdir}/src/mergeBAM.sh ${zumisdir} ${tmpMerge} ${num_threads} ${project} ${outdir} ${yaml} ${samtoolsexc}
  for i in ${fqfiles} ; do
  ...
  ${Rexc} ${zumisdir}/src/zUMIs-BCdetection.R ${yaml}

  #check if BC correction should be performed!
  BCbinRawTable=${outdir}/zUMIs_output/"${project}".BCbinning.raw.txt
  ...
  if [[ -f "${BCbinTable}" ]] ; then
      for x in ${l} ; do
        ...
        mv ${fixedbam} ${rawbam}
        if [[ "${chemistry}" == "MGI" ]] ; then
          perl ${zumisdir}/src/correct_BCtag_MGI.pl ${rawbam} ${fixedbam} ${BCbinRawTable} ${BCbinTable} ${samtoolsexc} &
        else
          perl ${zumisdir}/src/correct_BCtag.pl ${rawbam} ${fixedbam} ${BCbinTable} ${samtoolsexc} &
        fi
      done
      wait
  fi
...
[[ "${whichStage}" == "Mapping" ]] ; then
  echo "Mapping..."
    ${Rexc} ${zumisdir}/src/zUMIs-mapping.R ${yaml}
  date
fi
...
[[ "${whichStage}" == "Counting" ]] ; then
  echo "Counting..."
  ${Rexc} ${zumisdir}/src/zUMIs-dge2.R ${yaml}
  date
  ${Rexc} ${zumisdir}/src/misc/rds2loom.R ${yaml}
  date
  if [[ "${velo}" == "yes" ]] ; then
    ${Rexc} ${zumisdir}/src/runVelocyto.R ${yaml}
  fi
fi
...
[[ "${whichStage}" == "Summarising" ]] ; then
  if [[ "${isstats}" == "yes" ]] ; then
    echo "Descriptive statistics..."
      ${Rexc} ${zumisdir}/src/zUMIs-stats2.R ${yaml}
  fi
  date
fi
