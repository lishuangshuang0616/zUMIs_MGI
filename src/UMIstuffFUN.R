...
correct_UB_tags_new <- function(inbamfile,n){
  ...
  pypath <- paste0(opt$zUMIs_directory,"/src/correct_UBtag.py")
...
correct_UB_tags <- function(bccount, samtoolsexc){
  ...
  for(i in bccount$XC){
    ...
    pl_path <- paste0(opt$zUMIs_directory,"/src/correct_UBtag.pl")
...
demultiplex_bam <- function(opt, bamfile, nBCs, samtoolsexc, bccount){
  ...
  if(any(grepl("pysam==",installed_py))){
    ...
    py_script <- paste0(opt$zUMIs_directory,"/src/misc/demultiplex_BC.py")
    ...
    demux_cmd <- paste0(opt$zUMIs_directory,"/src/misc/demultiplex_BC.pl ",opt$out_dir," ",opt$project, " ", bamfile, " ", samtoolsexc )
    collect_demultiplex = FALSE
  }
  system(demux_cmd)
