...
setwd(opt$out_dir)
source(paste0(opt$zUMIs_directory,"/src/barcodeIDFUN.R"))
...
if(opt$barcodes$BarcodeBinning > 0 | !is.null(opt$barcodes$barcode_sharing | opt$chemistry == 'MGI')){
  result <- BCbin(bccount_file = paste0(opt$out_dir,"/", opt$project, ".BCstats.txt"),
                  bc_detected  = bccount)

  binmap <- result$binmap  
  binmap_raw <- result$binmap_raw
  fwrite(binmap,file=paste0(opt$out_dir,"/zUMIs_output/",opt$project,".BCbinning.txt"))
  fwrite(binmap_raw,file=paste0(opt$out_dir,"/zUMIs_output/",opt$project,".BCbinning.raw.txt"))
